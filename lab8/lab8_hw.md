...
```r
#These data are tidy! I know because each variable has its own column, each observation has its own row, and each value has its own cell. They are in long format. The only thing that is questionable to me is the date column, because the values entered are day/month/year in that format, but I don't think that qualifies as being more than one value in a cell, as the date refers to a single data point.
```
