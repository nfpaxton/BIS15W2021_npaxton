...
```r
fisheries_tidy %>% 
  summarise(across(c(country), n_distinct))
```
...
```r
fisheries_tidy %>% 
  summarise(across(c(asfis_species_number), n_distinct))
```
...
fisheries_tidy %>% 
  filter(year == 2000) %>% 
  group_by(country) %>% 
  summarise(overall_catch = sum(catch, na.rm = T), n_total = n()) %>% 
  arrange(desc(overall_catch))
```

```
## # A tibble: 193 x 3
##    country                  overall_catch n_total
##    <fct>                            <dbl>   <int>
##  1 China                            25899      93
...
fisheries_tidy %>% 
  ...
  group_by(country) %>% 
  summarise(sardine_record = sum(catch, na.rm = T), n_total = n()) %>% 
  arrange(desc(sardine_record))
```

```
## # A tibble: 37 x 3
##    country               sardine_record n_total
##    <fct>                          <dbl>   <int>
##  1 Morocco                         7470      22
##  2 Spain                           3507      33
...
fisheries_tidy %>% 
  ...
  group_by(country) %>% 
  summarise(cephalopod_catches = sum(catch, na.rm =T), n_total = n()) %>% 
  arrange(desc(cephalopod_catches)) %>% 
  head(cephalopod_catches, n = 5)
```

```
## # A tibble: 5 x 3
##   country cephalopod_catches n_total
##   <fct>                <dbl>   <int>
## 1 India                  570      10
## 2 China                  257       5
## 3 Spain                  198      10
...
fisheries_tidy %>% 
  ...
  summarise(most_caught_species = sum(catch, na.rm = T), n_total = n ()) %>%
  filter(asfis_species_name != "Osteichthyes") %>%
  arrange(desc(most_caught_species))
```

```
## # A tibble: 1,471 x 3
##    asfis_species_name    most_caught_species n_total
##    <chr>                               <dbl>   <int>
...
fisheries_tidy %>% 
  ...
  group_by(year) %>% 
  summarise(US_record_year = sum(catch, na.rm = T), n_total = n()) %>% 
  arrange(desc(US_record_year))
```

```
## # A tibble: 63 x 3
##     year US_record_year n_total
##    <dbl>          <dbl>   <int>
##  1  2004          19343     474
##  2  1997          19341     321
##  3  1996          18932     271
...
## # … with 53 more rows
```
I wanted to know what year the US had its largest overall catch, which was 2004. 
