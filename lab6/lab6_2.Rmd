...
#if there is even only 1 NA, NA is the output. Intolerant of NA, have to deal with them!
```{r}
penguins %>% 
  group_by(island) %>% 
...
```
#keep in mind: you can reverse order of variables
...
When you're doing Summarize function, you're making a new thing and have to name it! 
Risk of losing data, by removing NA via !is.na in filtering! Be careful and avoid this by individually removing NAs. 
```{r}
penguins %>% 
