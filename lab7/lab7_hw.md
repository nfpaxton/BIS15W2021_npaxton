...
```r
#It doesn't make sense for there to be 0 NA's because there are tons of -999s in amniota. The high number of NAs in amphibio makes sense because I saw a lot of NAs for each variable. More work needs to be done in amniota. 
```
...
```r
#Amphibio does not appear to need replacements
```
...
```r
#It makes sense that 4953 NA's exist for Mammalia egg mass because mammals give birth to live young, and don't lay eggs. That's why 100% of that data is NA. This is a good reminder that NA's don't necessarily mean missing data, but also are used for signifying that something is "not applicable." I am surprised that 92% of Reptilia egg mass data is NA though, as most reptiles do lay eggs. This seems like too many NAs.
```
...
```r
#We see that each species is entered 4 times, and that it can fall under one or more of the four categories of Fos, Ter, Aqu, Arb, with data missing from the categories that don't apply to it, hence NA = not applicable. The scientists used a binary system of 1 and 0 to indicate whether a variable was applicable to the species or not. 
```
