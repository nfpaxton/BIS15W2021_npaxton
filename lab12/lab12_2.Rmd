...
```{r}
lat <- c(34.67, 41.80) #concatonate min and max
long <- c(-124.1, -115.5) #concatonate min and max

bbox <- make_bbox(long, lat, f = 0.05) #bounding box, f is the way of slightly expanding bounding box beyond coordinates
```
...
```{r}
ggmap(map1) + 
  geom_point(data = spiders, aes(longitude, latitude), size = 0.25, color = "brown", shape = 6)+
  labs(x = "Longitude", y = "Latitude", title = "Spider Locations")
```
