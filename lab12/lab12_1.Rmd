...
```{r}
files <- list.files(path = "data/spiders", pattern = ".csv") #look for any .csv files
files
```
...
```{r}
day <- today()
str(day) #look at how R formats dates, structure
```
...
```{r}
dmy(spiders_with_locs$Date) #day month year
```
